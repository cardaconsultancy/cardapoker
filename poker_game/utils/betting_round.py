...
from poker_game.utils.pot_management import check_if_only_one_player_left
...
def betting_round_completed(table, preflop_round=False) -> bool:
    ...
    # this poses a problem when the last raiser has folded and is
    # automatically the last raiser after the BB
    big_blind_player = None

    while player != last_raiser:

        # if this is the first bet and we are not in the preflop round,
        # the first raiser is the one that sets the limit
        ...
        if preflop_round:
            ...
            # create a variable that keeps track of the big blind
            # player. We do this so that we can name the next player
            # automatically the last raiser without worrying about
        ...
        logger.debug("%s is up.", player.name)
        ...
        # UNLESS... the SB player has the highest bet because all the
        # other players cannot afford the SB... Ugh...
        # UNLESS... the round is not the preflop round
        logger.debug("the max bet is %s", max_bet)
        if (
        ...
        if not player.all_in and not player.folded:
            ...
            # ugly but this prevents the users from making mistakes (or
            # cheating) by this functionality in their standard function.
            ################## user call #####################
            player_bet = player.response(max_bet, player.hand, table)
            ##################################################

            # All in
            # Also check if the betsize is bigger than the chips, if so
            # correct by making it the max bet
            if (
                ...
                # check if the all in player is raising (seems better
                # than to check for all options if player is all in)
                if player.total_bet_betting_round > max_bet:
                    last_raiser = player
                    logger.debug("----%s is the last raiser", player.name)

            # Folding
            # Note that this automatically prevents a player from
            # folding when he/she can check, as it should
                ...
                # exception on the exception: if a SB player folds and
                # there are only two left, the next should not be BB,
                # because this is not how it works (it should end
                # there) and we do not take into account this
                # possibility
                # we can solve this by just checking if there is only
                # one other player (and possibly save some calc time
                # for the rest)
                # if len([player for player in table.players_game if
                # player.folded != True]) == 1:
                #     logger.debug("return True and let get and
                # check_if_rest_folded_and_pay pay the winner")
                #     return True
                if check_if_only_one_player_left(table):
                    return False

                # we don't want to remove the player from the game yet,
                # as we need to put his/her chips in the pot
        ...
        # to prevent a future issue with the player after the big blind
        # being automatically being the last raiser and folding:
        # but this leads to the issue that the player after the BB
        # might still need to call to a raise from the next player.

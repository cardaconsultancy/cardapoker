    ...
    def test_play_round(self):
        ...
        for game_seed in range(game_seed, game_seed + 2):
            ...
            player1 = create_player(
                "A", "raises_with_aces_reduces_with_12345", Chips(100)
            )
            ...
            nr_of_rounds += rounds
            logger.debug(f"Total  nr of rounds: {nr_of_rounds}")
            print(type(result_table))

            self.assertEqual(
                sum([player.chips.amount for player in table.players]), expected_sum
            )
